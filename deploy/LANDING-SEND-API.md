...
1. 在 `hero-split` / `card-stack` / `minimal-center` 中轮询（顺序带随机打散），找到 **已用次数 &lt; 上限** 的样式；
...
```json
{
  ...
  "used": 12,
  "limit": 100,
  "remainingByStyle": { "hero-split": 5, "card-stack": 88, "minimal-center": 100 },
  "note": "..."
}
```
...
`location ~ ^/(...|landing)(/|$)` 必须把 **`landing`** 列入，否则生产环境会得到 SPA 的 `index.html` 而非 Runner JSON。
